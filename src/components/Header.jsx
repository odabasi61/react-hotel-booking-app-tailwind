      ...
      <div className={type === 'list' ? 'headerContainer listMode' : 'headerContainer'}>
        ...
        <div className="headerSearch bg-white flex flex-col md:flex-row gap-2 md:gap-0 items-center border-2 border-[#febb02] justify-around py-2 md:py-5 rounded-sm absolute -bottom-16 md:-bottom-5 w-full max-w-[1024px] h-fit md:h-[30px]">
          ...
          <div className="headerSearchItem flex gap-2 items-center">
            ...
            <span onClick={() => setOpenDate(!openDate)} className="headerSearchText text-gray-600">{`${format(date[0].startDate, 'MM/dd/yyyy')} to ${format(date[0].endDate, 'MM/dd/yyyy')}`}</span>
            {openDate && <DateRange
          ...
          <div onClick={() => setOpenOptions(!openOptions)} className="headerSearchItem flex gap-2 items-center">
            <BsFillPersonFill className="headerIcon text-gray-600" />
            <span className="headerSearchText text-gray-600">
              {`${options.adult} adult · ${options.children} children · ${options.room} room`}
            </span>
            {openOptions && <div className="options absolute top-[100px] md:top-[40px] border-2 shadow-xl w-48 bg-white z-20">
              <div className="optionItem flex items-center justify-between gap-2 p-2 hover:bg-gray-300">
                ...
                <div className="flex gap-2 items-center">
                  <button className="optionCounterButton px-2 text-sm rounded-full border border-[#0071c2] text-black"
                    onClick={() => options.adult > 1 && handleOption('adult', 'd')}
                    // disabled={options.adult <= 1}
                  >-</button>
                  <span className="optionCounterNumber text-gray-600">{options.adult}</span>
                <button className="optionCounterButton px-2 text-sm rounded-full border border-[#0071c2] text-black" onClick={() => handleOption('adult', 'i')}>+</button>
                </div>
              </div>
              ...
              <div className="optionItem flex items-center justify-between gap-2 p-2 hover:bg-gray-300">
                ...
                <div className="flex gap-2 items-center">
                  <button className="optionCounterButton px-2 text-sm rounded-full border border-[#0071c2] text-black"
                  onClick={() =>options.children > 0 &&  handleOption('children', 'd')}>-</button>
                <span className="optionCounterNumber text-gray-600">{options.children}</span>
                  <button className="optionCounterButton px-2 text-sm rounded-full border border-[#0071c2] text-black"
              ...
              <div className="optionItem flex items-center justify-between gap-2 p-2 hover:bg-gray-300">
                ...
                <div className="flex gap-2 items-center">
                  <button className="optionCounterButton px-2 text-sm rounded-full border border-[#0071c2] text-black"
                  onClick={() =>options.room > 1 &&  handleOption('room', 'd')}>-</button>
                <span className="optionCounterNumber text-gray-600">{options.room}</span>
                  <button className="optionCounterButton px-2 text-sm rounded-full border border-[#0071c2] text-black"
